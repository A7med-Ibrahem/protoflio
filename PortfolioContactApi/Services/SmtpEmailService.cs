...
public class SmtpEmailService : IEmailService
{
    ...
    public async Task SendContactEmailAsync(ContactRequest request, CancellationToken cancellationToken = default)
    {
        ...
        {
            _logger.LogError(ex, "SMTP error while sending contact email. StatusCode: {StatusCode}", ex.StatusCode);
            throw new InvalidOperationException(
                $"SMTP failed with status '{ex.StatusCode}'. Verify SMTP host/port, Gmail App Password, and sender credentials.", ex);
        }
    }
    ...
    private static void ValidateSmtpSettings(SmtpSettings settings)
    {
        ...
        if (settings.Password.Contains("YOUR_APP_PASSWORD", StringComparison.OrdinalIgnoreCase))
        {
            throw new InvalidOperationException("SMTP password is still placeholder. Replace with a real Gmail App Password.");
        }
    }
}
